...
function getTopRatedMoviesInIndia() {
    // making http request
    ...
    function alertContents() {
        if (httpRequest.readyState == XMLHttpRequest.DONE) {
            if (httpRequest.status == 200) {
                var count = 0;
                // parsing data
                var topRatedIndiaMovies = JSON.parse(httpRequest.responseText);
                var topRatedIndiaMoviesDiv = document.getElementById('top-rated-inida-movies');
                var movieItem = '';
                // iterating data
                topRatedIndiaMovies.forEach((item, index) => {
                    ...
                    white-space: nowrap;
                    text-overflow: ellipsis;
                    max-width: 400px;"><strong style="color:#002b80">Rating: </strong>${item.contentRating}</p>
                    <p class="card-title" style="overflow: hidden;
                    white-space: nowrap;
                    text-overflow: ellipsis;
                ...
                topRatedIndiaMoviesDiv.innerHTML = movieItem;
            } else {
